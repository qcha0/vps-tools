#!/bin/bash
# xray一键安装脚本
# 原作者: hijk<https://github.com/hijkpw>
...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        if [[ -f ~/xray.pem && -f ~/xray.key ]]; then
            ...
            resolve=`curl -sL http://ip-api.com/json/${DOMAIN}?fields=query | awk -F'"' '{print $4}'`
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        echo "   2) 小说站(随机选择)"
        echo "   3) 壁纸站(https://www.cgwallpapers.com/)"
        echo "   4) 自定义反代站点(需以http或者https开头)"
        read -p "  请选择伪装网站类型[默认:高清壁纸站]" answer
        if [[ -z "$answer" ]]; then
            PROXY_URL="https://www.cgwallpapers.com"
        else
            case $answer in
            1)
                PROXY_URL=""
            ...
            2)
                ...
                do
                    ...
                    ip=`curl -sL http://ip-api.com/json/${host}?fields=query | awk -F'"' '{print $4}'`
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
		...
		# 邮箱写入无效地址
        curl -sL https://get.acme.sh | sh -s email=example@demo.sh
        source ~/.bashrc
